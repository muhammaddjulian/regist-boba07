...
const functionGenName = () => new Promise((resolve, reject) => {
	fetch('https://fakenametool.net/random-name-generator/male/id_ID/indonesia/1', {
		method: 'GET'
	})
	.then(res => res.text())
	.then(result => {
...
(async () => { 
	try {
        ...
        console.log(`[ ${moment().format('HH:mm:ss')} ] Set Pin Dan Registrasi Sukses`)

      

        //const dor = await functionDor(accessToken, uuid, uniqueid);
      	// const getAccessToken = await functionInputOtp(otpToken, otp, uuid, uniqueid);
       //  const access_token = JSON.parse(getAccessToken).data.accessToken
